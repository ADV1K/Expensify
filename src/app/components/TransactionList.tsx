...
export default function TransactionList({ transactions }: any) {
  const { setIsModalOpen }: any = useContext(ModalContext);
  ...
  return (
    <div className="space-y-1">
      {Object.entries(transactions).map(
        ([dateStr, { totalIncome, totalExpense, items }]: any) => (
          <div key={dateStr} className="card border">
            <div className="card-body gap-0 divide-y p-0">
              ...
              <div className="divide-y">
                {/* Transaction */}
                {items.map((transaction: any, index: any) => (
                  <div
                    key={index}
                    onClick={() => {
                      setIsModalOpen(true, transaction);
                    }}
